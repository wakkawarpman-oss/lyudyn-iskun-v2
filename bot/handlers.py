...
async def cmd_deep_osint(message: types.Message):
    ...
    try:
        ...
        # Prepare structured high-density context for military LLM
        ...
        sys_prompt = (
            ...
            "2. ЗАБОРОНЕНО давати поради військовим чи ППО (наприклад, 'рекомендується посилити ППО').\n"
            "3. Тільки конкретні факти: типи зброї, таймлайн хвиль, координати/райони, оцінка руйнувань, статус верифікації.\n"
            "4. Формат строго за 5 розділами нижче."
        )
...
async def handle_photo(message: types.Message, bot: Bot):
    ...
    def call_openai():
        ...
        sys_prompt = (
            ...
            "КРИТИЧНІ ПРАВИЛА:\n"
            "1. ОБОВ'ЯЗКОВО ПЕРЕВІР ВОДЯНІ ЗНАКИ, ЛОГОТИПИ, ГЕРБИ, ШЕВРОНИ ТА ТЕКСТ (наприклад: 'Одеська ОВА', 'ДСНС', дорожні знаки).\n"
            "2. ВКАЗУЙ РЕАЛЬНЕ МІСТО/РЕГІОН згідно знайдених водяних знаків та архітектури (НЕ прив'язуй автоматично до Києва, якщо на фото Одеса, Харків, Дніпро тощо).\n"
            "3. Жодної 'води' чи загальних роздумів про війну — тільки сухий військовий BDA-аналіз.\n\n"
            "ФОРМАТ ЗВІТУ:\n"
            "🏛 1. Атрибуція та Геолокація: [розпізнаний водяний знак ОВА/ДСНС/медіа, визначене місто/регіон, тип будівлі/об'єкта]\n"
            "🔥 2. Оцінка уражень (BDA): [епіцентр, поверхи, площа пошкодженого фасаду, вибите скло, стан залізобетонного каркасу, пожежа]\n"
            "🛡 3. Ймовірний тип озброєння: [характерні ознаки БпЛА Shahed-136/131 проти ракети чи падіння уламків]\n"
            "🕒 4. Хроно-аналіз та Анти-ІПСО: [час доби, світломаскування/освітлення у вікнах, ознаки монтажу чи архіву, оцінка автентичності X%]"
        )
        data = {
            "model": "gpt-4o-mini",
            "messages": [
