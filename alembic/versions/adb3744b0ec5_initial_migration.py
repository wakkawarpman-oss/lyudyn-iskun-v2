...
Revision ID: adb3744b0ec5
Revises: 
Create Date: 2026-09-02 15:15:48.793099
...
def upgrade() -> None:
    ...
    op.alter_column('detected_events', 'sources_list',
               existing_type=sa.TEXT(),
               type_=sa.String(),
               existing_nullable=True,
               existing_server_default=sa.text("''::text"))
    op.drop_index(op.f('idx_events_channel'), table_name='detected_events')
    op.drop_index(op.f('idx_events_detected_at'), table_name='detected_events')
    op.drop_index(op.f('idx_events_geom'), table_name='detected_events', postgresql_using='gist')
    op.drop_index(op.f('idx_events_resonance'), table_name='detected_events')
    op.drop_index(op.f('idx_events_verif'), table_name='detected_events')
    op.create_index(op.f('ix_detected_events_verification_status'), 'detected_events', ['verification_status'], unique=False)
    op.drop_index(op.f('idx_user_keys_uid'), table_name='user_api_keys')
    # ### end Alembic commands ###


def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.create_index(op.f('idx_user_keys_uid'), 'user_api_keys', ['user_id'], unique=False)
    op.drop_index(op.f('ix_detected_events_verification_status'), table_name='detected_events')
    op.create_index(op.f('idx_events_verif'), 'detected_events', ['verification_status'], unique=False)
    op.create_index(op.f('idx_events_resonance'), 'detected_events', [sa.literal_column('resonance_score DESC')], unique=False)
    op.create_index(op.f('idx_events_geom'), 'detected_events', ['geom'], unique=False, postgresql_using='gist')
    op.create_index(op.f('idx_events_detected_at'), 'detected_events', [sa.literal_column('detected_at DESC')], unique=False)
    op.create_index(op.f('idx_events_channel'), 'detected_events', ['source_channel'], unique=False)
    op.alter_column('detected_events', 'sources_list',
